...
class ColBERT(BaseColBERT):
    ...
    def __init__(self, name, colbert_config=None):
        ...
        if self.colbert_config.mask_punctuation:
            self.skiplist = {w: True
                             ...
                             for w in [symbol, self.raw_tokenizer(text=symbol, add_special_tokens=False)[0]]}
    ...
    # added by Jiaqi: allow colbert takes in query embeds processed by multimodal models.

    @torch.no_grad
    def query(self, Q_batch, attention_mask=None):
        with torch.autocast(device_type="cuda", dtype=self.model.dtype):
            Q, mask = self.model.get_q_retrieval_embed(**Q_batch)
            # Q = self.mapping(Q)
            #mask = torch.tensor(self.mask(input_ids, skiplist=[]), device=self.device).unsqueeze(2).float()
            mask = mask.unsqueeze(-1)
            Q = Q * mask
        
            return Q #torch.nn.functional.normalize(Q, p=2, dim=2)
    
    @torch.no_grad
    def doc(self, input_ids, attention_mask, keep_dims):
        assert keep_dims in [True, False, 'return_mask']
        input_ids, attention_mask = input_ids.to(self.device), attention_mask.to(self.device)
        with torch.autocast(device_type="cuda", dtype=self.model.dtype):
            D, mask = self.model.get_d_retrieval_embed(input_ids=input_ids, attention_mask=attention_mask)
            mask = mask.unsqueeze(-1)
            D = D * mask
...
def colbert_score(Q, D_padded, D_mask, config=ColBERTConfig()):
    ...
    with torch.autocast(device_type="cuda", dtype=self.model.dtype):
        ...
        score = colbert_score_reduce(scores, D_mask, config)
    return score
