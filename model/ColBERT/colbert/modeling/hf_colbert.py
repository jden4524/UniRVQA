...
from transformers import Blip2PreTrainedModel, Blip2Processor
...
base_class_mapping={
    ...
    "ybelkada/blip2-opt-2.7b-fp16-sharded": Blip2PreTrainedModel,
    "mmRAG/blip2-retrieve-merge": Blip2PreTrainedModel
}
...
model_object_mapping = {
    ...
    "ybelkada/blip2-opt-2.7b-fp16-sharded": Blip2ForConditionalGeneration,
    "mmRAG/blip2-retrieve-merge": Blip2ForConditionalGeneration
}
...
def class_factory(name_or_path):
    ...
    if getattr(loadedConfig, "auto_map", None) is None:
        ...
        if model_object_mapping.get(name_or_path) is not None:
    ...
    class HF_ColBERT(pretrained_class_object):
        ...
        def __init__(self, config, colbert_config):
            ...
            self.linear = nn.Linear(config.text_config.hidden_size, colbert_config.dim, bias=False)
        ...
        def from_pretrained(cls, name_or_path, colbert_config):
            ...
            obj = super().from_pretrained(name_or_path, colbert_config=colbert_config, device_map={"": 0}, load_in_4bit=True, torch_dtype=torch.float16, bnb_4bit_compute_dtype=torch.float16)
        ...
        #     obj = Blip2Processor.from_pretrained('Salesforce/blip2-opt-2.7b').tokenizer
        #     obj.base = name_or_path
        #     obj.pad_token = obj.eos_token
