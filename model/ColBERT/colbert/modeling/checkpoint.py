...
class Checkpoint(ColBERT):
    ...
    # def query_emb(self, *args, to_cpu=False, **kw_args):
    #     with torch.no_grad():
    #         with self.amp_manager.context():
    #             Q = super().query_emb(*args, **kw_args)
    #             return Q.cpu() if to_cpu else Q

    def doc(self, docs, to_cpu,keep_dims, **kw_args):
        with torch.no_grad():
            with self.amp_manager.context():
                D, mask = super().doc(docs['input_ids'], docs['attention_mask'], keep_dims=keep_dims, **kw_args)
    ...
    def docFromText(self, docs, to_cpu=False, keep_dims="return_mask"):
        D, mask = self.doc(docs, to_cpu, keep_dims)
