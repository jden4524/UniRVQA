...
from transformers import Blip2Processor, InstructBlipProcessor


class QueryTokenizer():
    def __init__(self, config: ColBERTConfig, verbose: int = 3):
        # HF_ColBERT = class_factory(config.checkpoint)
        if "blip2" in config.model_name:
            self.tok = Blip2Processor.from_pretrained(config.model_name).tokenizer
        elif "instruct" in config.model_name:
            self.tok = InstructBlipProcessor.from_pretrained(config.model_name).tokenizer
        self.tok.base = config.model_name
