...
class Searcher:
    ...
    def search_all(self, queries, k=10, filter_fn=None, full_length_search=False, qid_to_pids=None, qid_batch=None):
        #queries = Queries.cast(queries)
        #queries_ = list(queries.values())

        Q = self.encode(queries, full_length_search=full_length_search)

        return self._search_all_Q_new(Q, k, filter_fn=filter_fn, qid_to_pids=qid_to_pids, qid_batch=qid_batch)
    ...
    def _search_all_Q_new(self, Q, k, filter_fn=None, qid_to_pids=None, qid_batch=None):
        qids = list(qid_batch)
